...
- **Round admins may submit projects to their own round (explicit decision).** `submit_project` has no check preventing the round creator/admins from also being a submitter. Considered and deliberately not restricted: the scoring itself is produced by `gl.eq_principle.prompt_non_comparative` reading the submitted evidence, which an admin cannot bias any more than any other submitter can by writing a stronger submission; the actual conflict-of-interest surface (the pool amount entered into `compute_distribution`, and the timing of `close_round`) is already fully on-chain, admin-attributed, and auditable regardless of who submitted. Restricting self-submission would also break a legitimate case (a round creator submitting a reference/example project). Both the submit form and the project page surface this explicitly rather than hiding it: the submit form shows a note when the connected admin selects their own round, and the project page shows "Submitted by a round admin" to every viewer when applicable.
...
- **Versioned history, not silent overwrite:** every re-evaluation increments a `version` field and pushes the prior evaluation (score, confidence, dimension scores, reasoning, citations, grounding notes, challenger) into a bounded `history` list on the same record. The full dispute trail is on-chain and shown in the UI, not destroyed.
- **Admin restore (`restore_evaluation`):** a bad challenge can legitimately replace a good evaluation with a worse one. A round admin can promote any prior version from history back to current, but only before distribution freezes results. The restore never deletes history: the replaced evaluation is itself snapshotted, the restored record carries `restored_from` and `restored_by`, and `challenge_count` is not reset (restoring does not refund challenge attempts). The UI shows a visible "Restored from vN by round admin" badge to every viewer.
...
- **Fact grounding (best effort within GenLayer's current primitives):** the prompt requires the model, for every dimension score, to (1) state which evidence indices it accessed and whether each loaded, (2) quote or closely paraphrase a specific concrete piece of retrieved content, and (3) explain how that content supports or fails to support the score, with explicit instructions to declare inaccessible or unsupportive evidence rather than invent support. The model also returns per-evidence `evidence_notes` (accessibility + a short factual description of what was found), which the contract index-binds to the actual submitted URLs (`_bind_evidence_notes`) and stores for display. The Equivalence Principle criteria instructs validators to reject reasoning that is only vague or generic praise with no concrete reference to evidence content, and to check per-index accessibility notes. **Known boundary, stated plainly:** GenLayer does not currently provide cryptographic verification of what a URL's content actually was at evaluation time, so these measures raise the cost of ungrounded scoring rather than making it impossible; validator agreement under the Equivalence Principle is the enforcement mechanism available today.
...
- **Concentration safeguard (`max_share_bps`):** purely proportional splitting would let one high-scoring project absorb nearly the whole pool. `compute_distribution(round_id, pool, max_share_bps)` caps any single project's share in basis points (frontend default 4000 = 40%; 10000 disables the cap; validated to the 100..10000 range). Amounts freed by the cap are redistributed proportionally among uncapped projects via deterministic, iterative integer arithmetic that terminates in at most one pass per project. If capping leaves part of the pool unassignable (e.g. a single project under a 40% cap), the remainder is recorded on the round as `undistributed` rather than silently disappearing, and the cap used is stored on the round (`max_share_bps`) for auditability.
